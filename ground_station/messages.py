...
# Our project-specific mapping from SDL gamepad button constants
# sdl::SDL_GamepadButton (https://wiki.libsdl.org/SDL3/SDL_GamepadButton)
# to our custom button-codes (see comp team work package 2)
SDL_BUTTONS_TO_BUTTON_CODES = {
    sdl3.SDL_GAMEPAD_BUTTON_WEST: 1 << 0,  # Strafe Left
    sdl3.SDL_GAMEPAD_BUTTON_EAST: 1 << 1,  # Strafe Right
    sdl3.SDL_GAMEPAD_BUTTON_LEFT_SHOULDER: 1 << 2,  # Sprint
    # Left trigger :: 1 << 3 :: Use
    sdl3.SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER: 1 << 4,  # Next Weapon
    # Right trigger :: 1 << 5 :: Fire
    sdl3.SDL_GAMEPAD_BUTTON_NORTH: 1 << 6,  # Map
}

SDL_AXES_TO_BUTTON_CODES = {
    sdl3.SDL_GAMEPAD_AXIS_LEFT_TRIGGER: 1 << 3,  # Use
    sdl3.SDL_GAMEPAD_AXIS_RIGHT_TRIGGER: 1 << 3,  # Fire
}


class DOOMGamepadTick:
    ...
    def is_idle(self) -> bool:
        return not bool(self.x or self.y or self.buttons)

    def __repr__(self):
        return f"DOOMGamepadTick(x={self.x}, y={self.y}, buttons={self.buttons:#b})"

    def __bytes__(self):
        """
        Serializes the keystroke into 5 bytes.

        Format: [buttons][X][Y] (Big-Endian).
        """
        return struct.pack("!Bhh", self.buttons, self.x, self.y)


class DOOMGamepadTickList(MutableSequence[DOOMGamepadTick]):
    def __init__(self, ticks: Iterable[DOOMGamepadTick] | None = None):
        self._ticks: list[DOOMGamepadTick] = list(ticks) if ticks else []

    @overload
    def __getitem__(self, index: int) -> DOOMGamepadTick: ...
    @overload
    def __getitem__(self, index: slice) -> MutableSequence[DOOMGamepadTick]: ...
    def __getitem__(self, index):
        return self._ticks.__getitem__(index)

    @overload
    def __setitem__(self, index: int, value: DOOMGamepadTick) -> None: ...
    @overload
    def __setitem__(self, index: slice, value: Iterable[DOOMGamepadTick]) -> None: ...
    def __setitem__(self, index, value):
        return self._ticks.__setitem__(index, value)

    def __delitem__(self, index: int | slice) -> None:
        self._ticks.__delitem__(index)

    def __len__(self) -> int:
        return len(self._ticks)

    def insert(self, index: int, value: DOOMGamepadTick) -> None:
        self._ticks.insert(index, value)

    @property
    def size_in_bytes(self) -> int:
        """
        Returns the byte length of the keystroke data payload (5 bytes per entry).
        Does not include the 1-byte length header used in __bytes__.
        """
        return len(self._ticks) * 5

    def remove_trailing_idles(self):
        """
        Truncates all idle (empty) ticks from the end of the sequence
        to save bandwidth/space.
        """
        while self._ticks and self._ticks[-1].is_idle():
            self._ticks.pop()

    def __repr__(self):
        items = "\n".join(f"    {k!r}," for k in self._ticks)
        return f"DOOMGamepadTickList([\n{items}\n])"

    def split_to_serialise(self) -> list[DOOMGamepadTickList]:
        """
        Splits the list into chunks of 255 keystrokes.
        ...
        return [
            DOOMGamepadTickList(self[i : i + 255]) for i in range(0, len(self), 255)
        ]
    ...
    def __bytes__(self):
        ...
        Format:
            - Byte 0: Number of gamepad ticks (N) as an unsigned 8-bit integer.
            - Bytes 1 to (N*5): Gamepad data.

        Raises:
            ValueError: If the list contains more than 255 gamepad ticks.
        """

        if len(self) > 255:
            raise ValueError(
                "DOOMGamepadTickList exceeds maximum serializable length (255)."
            )

        entries = b"".join(bytes(k) for k in self._ticks)
        return struct.pack("!B", len(self)) + entries
